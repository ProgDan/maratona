...
public class Main {
	...
	public static void main(String[] args) {
		...
		int n = Integer.parseInt(sc.nextLine());	//n�mero de casos de teste
		for (int i=0 ; i<n ; i++) {
			
			int t = Integer.parseInt(sc.nextLine());//n�mero de compostos perigosos poss�veis
			String[] formula = new String[t];
			for (int x=0 ; x<t ; x++)
				formula[x] = sc.nextLine();
			
			int u = Integer.parseInt(sc.nextLine());//quantidade experiencias que Juvenal ir� realizar
	...
	private static boolean isNotNumber(String string) {
		...
		//System.out.println("� numero " + Character.isDigit(c[0]));
		return Character.isDigit(c[0]);
	}

	private static boolean isLowerCase(String string) {
		//System.out.println("� minusculo " + string.toLowerCase().equals(string));
